...
public class OrderBookProvider extends ContentProvider {
	...
	private static final String CREATE_ORDERBOOKURL_TABLE_SQL = "CREATE TABLE" + " " + OrderBookProviderContract.ORDERBOOK_TABLE_NAME + " "
	        + "(" + " " + OrderBookProviderContract.ROW_ID + " " + PRIMARY_KEY_TYPE + " ,"
	        + OrderBookProviderContract.ORDERBOOK_TIMESTAMP_COLUMN + " " + INTEGER_TYPE + " ,"
	        + OrderBookProviderContract.ORDERBOOK_KIND_COLUMN + " " + TEXT_TYPE + " ," + OrderBookProviderContract.ORDERBOOK_PRICE_COLUMN
	        + " " + TEXT_TYPE + " ," + OrderBookProviderContract.ORDERBOOK_AMOUNT_COLUMN + " " + TEXT_TYPE + ")";
	...
	static {
		...
		sUriMatcher.addURI(OrderBookProviderContract.AUTHORITY, OrderBookProviderContract.ORDERBOOK_TABLE_NAME, ORDERBOOK_URL_QUERY);

		// Specifies a custom MIME type for the picture URL table
		sMimeTypes.put(ORDERBOOK_URL_QUERY, "vnd.android.cursor.dir/vnd." + OrderBookProviderContract.AUTHORITY + "."
	...
	private class OrderBookProviderHelper extends SQLiteOpenHelper {
		...
		OrderBookProviderHelper(Context context) {
			super(context, OrderBookProviderContract.DATABASE_NAME, null, OrderBookProviderContract.DATABASE_VERSION);
		}
		...
		public void onUpgrade(SQLiteDatabase db, int version1, int version2) {
			Log.w(OrderBookProviderHelper.class.getName(), "Upgrading database from version " + version1 + " to " + version2
		...
		public void onDowngrade(SQLiteDatabase db, int version1, int version2) {
			Log.w(OrderBookProviderHelper.class.getName(), "Downgrading database from version " + version1 + " to " + version2
			        + ", which will destroy all the existing data");
	...
	public Cursor query(Uri uri, String[] projection, String selection, String[] selectionArgs, String sortOrder) {
		...
		Cursor returnCursor = db.query(OrderBookProviderContract.ORDERBOOK_TABLE_NAME, projection, selection, selectionArgs, groupBy,
		        having, sortOrder);
	...
	public Uri insert(Uri uri, ContentValues values) {
		...
		long id = localSQLiteDatabase.insert(OrderBookProviderContract.ORDERBOOK_TABLE_NAME, null, values);
	...
	/**
	 * Implements bulk row insertion using {@link SQLiteDatabase#insert(String, String, ContentValues) SQLiteDatabase.insert()} and SQLite
	 * transactions. The method also notifies the current {@link android.content.ContentResolver} that the
	 * {@link android.content.ContentProvider} has been changed.
	 * 
	...
	public int bulkInsert(Uri uri, ContentValues[] insertValuesArray) {
		...
		for (int i = 0; i < numOrderBook; i++) {
			...
			localSQLiteDatabase.insert(OrderBookProviderContract.ORDERBOOK_TABLE_NAME, null, insertValuesArray[i]);
		}
	...
	public int update(Uri uri, ContentValues values, String selection, String[] selectionArgs) {
		...
		// Updates the table
		int rows = localSQLiteDatabase.update(OrderBookProviderContract.ORDERBOOK_TABLE_NAME, values, selection, selectionArgs);
