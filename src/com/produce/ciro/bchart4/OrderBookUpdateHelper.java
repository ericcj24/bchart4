...
public class OrderBookUpdateHelper {
	...
	protected void secondCall() {
		...
		pusher.connect(new ConnectionEventListener() {
			...
			public void onConnectionStateChange(ConnectionStateChange change) {
				Log.i(TAG, "State changed to " + change.getCurrentState() + " from " + change.getPreviousState());
			}
	...
	private class LogTask extends AsyncTask<Void, Void, Void> {
		...
		private void addNewSingleOrderbook(Orderbook1 ob) {
			...
			Cursor cursor = cr.query(OrderBookProviderContract.CONTENT_URI, projection, selection, selectionArgs, sortOrder);
	...
	private OrderBook fetchOrderbook() {
		...
		try {
			...
			// consider move c to global scale
			// c.disconnect();
		}
		return ob;
	}
